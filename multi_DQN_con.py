    ...
    with tqdm(total=int(num_episodes / 10), desc='Iteration %d' % i) as pbar:
        for i_episode in range(int(num_episodes / 10)):
            ...
            while not done:
                # print(state)
                # exit(0)
                action1 = agent1.take_action(state[0])
                ...
                replay_buffer1.add(state[0], action[0], reward[0], next_state[0], done[0])
                replay_buffer2.add(state[1], action[1], reward[1], next_state[1], done[1])
                replay_buffer3.add(state[2], action[2], reward[2], next_state[2], done[2])
                replay_buffer4.add(state[3], action[3], reward[3], next_state[3], done[3])
                done = all(done)
                state = next_state
