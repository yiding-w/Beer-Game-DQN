...
class BeerGame(gym.Env):
    ...
    def step(self, action:list,num=4):
    ...
    def handleAction(self, action,num=4):
        ...
        for k in range(0, self.config.NoAgent):
            ...
            if k<=num:
                # print(self.config.actionListLenOpt)
                self.players[k].action = np.zeros(61)        #one-hot transformation
    ...
    def next(self):
        ...
        for k in range(self.config.NoAgent-1,-1,-1): # [3,2,1,0]
            ...
            self.players[k].IL -= possible_shipment
    ...
    def getAction(self, k):
        ...
        # if self.players[k].IL < 5:
        #     # print(self.players[k].IL)
        #     self.players[k].action[min(60,5-int(self.players[k].IL))] = 1
        # else:
        #     self.players[k].action[0] = 1
        # print(self.config.actionListLenOpt)
        # exit(0)
